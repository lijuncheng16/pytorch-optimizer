...
def mapping_function(input, nbit):
    pass

class Quant(Optimizer):
    ...
    def __init__(
        ...
        super(Quant, self).__init__(params, defaults)
    ...
    def step(self, closure: OptLossClosure = None) -> OptFloat:
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                exp_avg = exp_avg.type(torch.float16)
                exp_avg = exp_avg.type(torch.float32)
                denom = denom.type(torch.float16)
                denom = denom.type(torch.float32)
#                 torch.cast(exp_avg, dtype=torch.int8)
#                 torch.cast(exp_avg, dtype=torch.float32)
#                 torch.cast(denom, dtype=torch.int8)
#                 torch.cast(denom, dtype=torch.float32)
#                 exp_avg.float32()
#                 denom.int8()
#                 denom.float32()
